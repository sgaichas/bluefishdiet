...
1.  Spatial and spatio-temporal random effects plus anisotropy in both linear predictors;  "_alleffectson"; FieldConfig default (all IID)
1.  Spatial and spatio-temporal random effects in both linear predictors with anisotopy turned off;  "_noaniso" ; FieldConfig default (all IID) and `use_anisotopy = FALSE`
1.  Spatial and spatio-temporal random effects plus anisotropy only in linear predictor 1;  "_noomeps2"; FieldConfig = 0 for Omega2, Epsilon2
1.  Spatial and spatio-temporal random effects only in linear predictor 1 with anisotopy turned off; "_noomeps2_noaniso";  FieldConfig = 0 for Omega2, Epsilon2 and `use_anisotopy = FALSE`
1.  Spatial random effects plus anisotropy only in linear predictor 1;  "_noomeps2_noeps1"; FieldConfig = 0 for Omega2, Epsilon2, Epsilon1
1.  Spatial random effects only in linear predictor 1 with anisotopy turned off;  "_noomeps2_noeps1_noaniso"; FieldConfig = 0 for Omega2, Epsilon2, Epsilon1 and `use_anisotopy = FALSE`
1.  Anisotropy, but no spatial or spatio-temporal random effects in either linear predictor;  "_noomeps12"; FieldConfig = 0 for both Omega, Epsilon 
1.  No spatial or spatio-temporal random effects in either linear predictor;  "_noomeps12_noaniso"; FieldConfig = 0 for both Omega, Epsilon  and `use_anisotopy = FALSE`
