---
title: '2022 Bluefish (*Pomatomus saltatrix*) Research Track Working Paper YY: Forage
  Index'
...
flextable::flextable(blueprey[,c('pynam','BLUEFISH')]) %>%
  flextable::set_header_labels(pynam = "Prey name", BLUEFISH = "Bluefish stomachs (n)") %>%
...
### VAST modeling  
We will use VAST [@thorson_comparing_2017; @thorson_guidance_2019] to evaluate changes in bluefish prey biomass and distribution over time. VAST is structured to estimate fixed and random effects across two linear predictors, which are then multiplied to estimate an index of the quantity of interest. A full model including fixed intercepts ($\beta$), spatial random effects ($\omega$), spatio-temporal random effects ($\varepsilon$), vessel effects ($\eta$), density or habitat covariates, and catchability covariates. 

$$ \rho_1(i) = \beta_1(c_i, t_i) + \omega_1^*(s_i, c_i) + \varepsilon_1^*(s_i, c_i, t_i) + \eta_1(v_i, c_i) + \nu_1(c_i, t_i) + \zeta_1(i) - \iota(c_i, t_i) $$

#### Structural decisions  
@thorson_guidance_2019 lists 15 major decisions for constructing a VAST model. 

##### Spatial domain
Models were run using the full Northwest Atlantic grid built into VAST. 
...
While all strata were applied in all models, the survey strata-based definitions are most relevant to the seasonal models, as seasonal survey information is used in the bluefish assessment. The State and Federal waters strata are most relevant to the annual model, as annual recreational catch per unit effort information is used in the bluefish assessment. 
...
Comparisons of AIC are presented for both the first (spatial and spatio-temporal random effects) and second (catchability covariates) rounds of model selection. 
...
Using REML, models including spatial and spatio-temporal random effects as well as anistropy were best supported by the data. This was true for the spring dataset, the fall dataset, and the annual (seasons combined) dataset. 
...
Survey strata definitions are built into VAST already. The results presented here have all splits in one with a newly defined strata set. This required use of the development version of VAST, because the 3 mile split cuts across survey stratum boundaries. 

Definitions for strata used in the script `bluefishdiet/VASTunivariate_bfp_allsurvs_lencovSST_ALLinoffsplits.R`:

```{r}
# consider putting plots of strata sets on maps here
```

Bias correction is based on [@thorson_implementing_2016]. But we are using the new implementation in the VAST dev branch code. 
...
For annual index, run this script, the same used to test catchability covariates, but now has all three covariates (predator mean length, number of predator species, and SSTfill) included and  `bias.correct=TRUE`: 

```{r, code = readLines(here("VASTunivariate_bfp_allsurvsANNUAL_lencovSST_ALLinoffsplits.R")), eval=F}
```
...
![FAnnual density maps with covariates](pyindex/allagg_annual_500_lennosst_ALLsplit_biascorrect/ln_density-predicted.png)
...
All results in the respective `pyindex/allagg_fall_500_lennosst_ALLsplit_biascorrect`, `pyindex/allagg_spring_500_lennsst_ALLsplit_biascorrect` and `pyindex/allagg_annual_500_lennsst_ALLsplit_biascorrect` folders.
...
Current inputs for WHAM are processed with the following script:
