...
Can I go right from daily nc to dataframe?
...
for(i in years) {
  ...
  name <- paste0(stringr::str_extract(yrfiles[1], "^.{14}"), ".nc")
  download.file(paste0(url, yrfiles[1]), destfile = name)
  
  r <- terra::rast(name, "sea_surface_temperature")
  rcrop <- terra::crop(r, c(-77,-65,35,45))
  sstdf <- terra::as.data.frame(rcrop, xy=TRUE) %>% 
    mutate(sst = sea_surface_temperature - 273.15) %>%
    filter(sst > -3)

  #one big list of year month matches
  allfiles <- bind_rows(allfiles, yrfiles)
}


ggplot() +
    geom_tile(data = sstdf, aes(x = x, y = y, fill = sst)) +
    geom_sf(data = ecodata::coast) +
    scale_fill_gradientn(name = "Temp C",
                         limits = c(0.5, 31),
                         colours = c(scales::muted("blue"), "white",
