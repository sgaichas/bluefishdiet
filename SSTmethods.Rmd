...
stations <- bluepyagg_stn_all %>%
  dplyr::mutate(yrmody = paste0(year, month, day)) %>%
  dplyr::select(id, declon, declat, yrmody) %>%
  na.omit() %>%
...
for(df in SSTdfs){
  ...
  sstdf_survdays <- sstdf %>%
    dplyr::mutate(yrmody = paste0(year, month, day)) %>%
    dplyr::filter(yrmody %in% stations$yrmody) %>%
  ...
  # find nearest SST to survey locations
  #nearest_sst <- st_nn(stations, sstdf_survdays, returnDist = T, k = 1, progress = FALSE)

    
  yrdietOISST <- inner_join(bluepyagg_stn_all, sstdf_survdays) %>%
