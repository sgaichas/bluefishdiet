...
NEAMAPprey <- NEAMAPblueprey %>%
  dplyr::select(SCIENTIFIC.NAME, BLUEFISH) %>%
...
NEFSCprey <- NEFSCblueprey %>%
  dplyr::select(pynam, BLUEFISH) %>%
  dplyr::filter(!is.na(BLUEFISH)) %>%
  dplyr::mutate(pynam2 = tolower(pynam)) %>%
  dplyr::rename(NEFSC = BLUEFISH)

blueprey <- NEFSCprey %>% 
  dplyr::left_join(NEAMAPprey) %>%
  dplyr::mutate(NEAMAP = ifelse(is.na(NEAMAP), 0, NEAMAP),
                total = NEFSC + NEAMAP) %>%
  dplyr::arrange(desc(total)) %>%
  dplyr::filter(total>9)

flextable::flextable(blueprey[,c('pynam', 'NEFSC', 'NEAMAP', 'total')]) %>%
  flextable::set_header_labels(pynam = "Prey name",  total = "Bluefish stomachs (n)")
