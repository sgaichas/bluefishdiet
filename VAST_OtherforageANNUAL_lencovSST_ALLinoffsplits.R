...
bluepyagg_stn <- readRDS(here::here("fhdat/bluepyagg_stn_all_OISST_unmod.rds"))
...
working_dir <- here::here(sprintf("pyindex_unmod/allagg_%s/", season))
...
#bluepyagg_stn_all <- bluepyagg_stn_all %>% filter(Year<1990)                       

fit <- fit_model(
  settings = settings, 
  extrapolation_list = New_Extrapolation_List,
  Lat_i = bluepyagg_stn_all$Lat, 
  Lon_i = bluepyagg_stn_all$Lon, 
  t_i = bluepyagg_stn_all$Year, 
  b_i = as_units(bluepyagg_stn_all[,'Catch_g'], 'g'),
  a_i = rep(1, nrow(bluepyagg_stn_all)),
  v_i = bluepyagg_stn_all$Vessel,
  Q_ik = as.matrix(bluepyagg_stn_all[,c("npiscsp", 
                                         "meanpisclen", 
                                         "sstfill"
                                         )]),
  #Use_REML = TRUE,
